...
def start(player_side):
    ...
    pygame.init()
    pygame.display.set_caption('Игра в шахматы')
    ...
    while True:
        ...
        if mode == 'mode_5':
            ...
            # Если игра не завершена, то перейти в режим 1
            mode = 'mode_1'
...
# Функция добавляет к кол-ву выигранных партий +1, присваивает 1 очко
def update_wingame():
    sql = "SELECT wingame, points FROM user_info where userid = %s"
    cursor.execute(sql, (config.id, ))
    wingame = cursor.fetchone()
    wingameBefore = wingame[0]   
    wingameAfter = wingameBefore + 1
    cursor.execute("UPDATE user_info SET wingame =%s WHERE userid =%s", (wingameAfter, config.id))
    pointsBefore = wingame[1]
    pointsAfter = pointsBefore + 1
    cursor.execute("UPDATE user_info SET points =%s WHERE userid =%s", (pointsAfter, config.id))
    conn.commit()


# Функция добавляет к кол-ву проигранных партий +1
def update_failgame():
    sql = "SELECT failgame FROM user_info where userid = %s"
    cursor.execute(sql, (config.id, ))
    failgame = cursor.fetchone()
    failgameBefore = failgame[0]
    failgameAfter = failgameBefore + 1
    cursor.execute("UPDATE user_info SET failgame =%s WHERE userid =%s", (failgameAfter, config.id))
    conn.commit()
 

# Функция добавляет 0.5 очков за ничью
def dead_heat():
    sql = "SELECT points FROM user_info where userid = %s"
    cursor.execute(sql, (config.id, ))
    points = cursor.fetchone()
    pointsBefore = points[0]
    pointsAfter = pointsBefore + 0.5
    cursor.execute("UPDATE user_info SET points =%s WHERE userid =%s", (pointsAfter, config.id))
    conn.commit()
